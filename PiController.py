...
def startDocker():
    ...
    envVariables={"dockerIP":dockerExternalIP,"dockerPort":dockerPort,"serviceIP":serviceIP,"servicePort":servicePort,"accessKey":config.get('default', 'aws_access_key_id'),"secretKey":config.get('default', 'aws_secret_access_key'),"sessionKey":config.get('default', 'aws_session_token'),"bucketName":'disnetlab'}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if localBatteryLevel != None:
            if int(localBatteryLevel) > 50:
                if localDockerStatus==0 and remoteDockerStatus==0:
                    serviceList=["motion"]
                    ...
                    if responseData==True:
                        ...
                        logger.info('Controller: local docker creation in failed')
                elif localDockerStatus==0 and remoteDockerStatus==1 and int(localBatteryLevel) > 60:
                ...
                if remoteDockerStatus==0:
                    neighbourPIs = checkNeighbours()
                    for PI in neighbourPIs:
                        print(PI)
                        remoteBatteryLevel = getBatteryOfPIs(PI,servicePort)
                        if remoteBatteryLevel != None and int(remoteBatteryLevel)>70:
                            selectedRemotePi = PI
                            logger.info('Controller: '+str(selectedRemotePi)+' is selected for docker exectuion')
                            break
                    serviceList=["motion"]
                    responseData = requestDockerStart(selectedRemotePi,controllerPort,externalIP,servicePort,imageName,serviceList)
                    if responseData==True:
                        logger.info('Controller: remote docker is running')
                        remoteDockerStatus=1
                        for key in ipDockerNameBind:
                            print(key)
                            responseData1 = requestDockerStop(externalIP,controllerPort,key)
                        logger.info('Controller: terminated all locally run docker')
                    else:
                        logger.info('Controller: remote docker creation in failed')
                    time.sleep(30)     
